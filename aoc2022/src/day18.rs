...
fn neighbors(pos: &Droplet) -> impl Iterator<Item = Droplet> + '_ {
    ...
    .map(|c| {
        (
            (pos.0 as i32 + c.0) as usize,
            (pos.1 as i32 + c.1) as usize,
            (pos.2 as i32 + c.2) as usize,
        )
    })
}
...
fn flood(grid: &mut Vec<Vec<Vec<State>>>, drop: &Droplet) {
    if drop.0 >= grid.len()
        || drop.1 >= grid[0].len()
        || drop.2 >= grid[0][0].len()
        || grid[drop.0][drop.1][drop.2] != State::Air
    {
        return;
    }
...
fn part1(file: &str) {
    ...
    droplets
        .iter()
        .for_each(|d| grid[d.0][d.1][d.2] = State::Lava);
    println!(
        "Part 1: {}",
        count_surfaces_with_state(&grid, State::Air, &droplets)
    );
}
